...
class feature_gen_multi(keras.utils.Sequence):
    ...
    def __init__(self,Dpath,E_path,N_path,Z_path,y_path,EQinfo,STAinfo,Nstan=121,add_code=True,add_noise=True,noise_p=0.5,  
        ...
        self.shuffle = shuffle  #shuffle always True (shuffle station and eqs?)
        #self.__check_shape__()
        self.savePSD = {} # initial PSD for memorization
    def __len__(self):
        #length
        return len(self.E_path)
    def __check_shape__(self):
                    ...
                    if add_noise:
                        ...
                        for n in range(Nstan):
                            if n in rndSTAidx[:rm_Nstan]:
                                continue
                            np.random.shuffle(level) #randomly pick from the level list
                            if level[0] not in self.savePSD:
                                f,Epsd,Npsd,Zpsd = gnss_psd(level=level[0],return_as_frequencies=True,return_as_db=False)
                                self.savePSD[level[0]] = (f,Epsd,Npsd,Zpsd)
                            f,Epsd,Npsd,Zpsd = self.savePSD[level[0]]
                            Noise_add_E,Noise_add_N,Noise_add_Z = make_noise(tmp_E.shape[0],f,Epsd,Npsd,Zpsd,PGD=False)
                            tmp_E[:,n] = tmp_E[:,n] + Noise_add_E
                            tmp_N[:,n] = tmp_N[:,n] + Noise_add_N
                            tmp_Z[:,n] = tmp_Z[:,n] + Noise_add_Z
...
class feature_gen_specific(keras.utils.Sequence):
    # same as feature_gen, but only return specific data (by time index). Note the data size is [batch_size, time, features]
    # *** generator has NOT been optimized yet, however, given the parallel option in traning it is fast
    def __init__(self,Dpath,E_path,N_path,Z_path,y_path,EQinfo,STAinfo,Nstan=121,add_code=True,add_noise=True,noise_p=0.5,  
                 rmN=(10,110),Noise_level=[1,10,20,30,40,50,60,70,80,90],Min_stan_dist=[4,3],scale=(0,1), 
                 BatchSize=128,Mwfilter=8.0,save_ID='sav_pickedID_1_valid.npy',time_id=0,shuffle=True):
        self.Dpath=Dpath #The path of the individual [E/N/U].npy data (should be a list or a numpy array)
        self.E_path=E_path #The path of the individual [E/N/U].npy data (should be a list or a numpy array)
        self.N_path=N_path
        self.Z_path=Z_path
        ...
        self.Mwfilter=Mwfilter #Threshold magnitude for generated events, or False means everything
        self.save_ID=save_ID #save the original eqid with this name (e.g. sav_pickedID_73_2_valid.npy)
        self.time_id = time_id # only return data at specific time index
        self.shuffle=shuffle  #shuffle always True (shuffle station and eqs?)
        #self.__check_shape__()
    def __len__(self):
        #length
    ...
    def __getitem__(self,index):
        ...
        index is useless here since I want every batches to be different
        '''
        Dpath,E,N,Z,y,EQinfo,STAinfo,Nstan,add_code,add_noise,noise_p,rmN,level,Min_stan_dist,scale,BatchSize,Mwfilter,save_ID,time_id,shuffle= \
        (self.Dpath,self.E_path,self.N_path,self.Z_path,self.y_path,self.EQinfo,self.STAinfo,self.Nstan,self.add_code,self.add_noise,self.noise_p,self.rmN,self.Noise_level,self.Min_stan_dist,self.scale,self.BatchSize,self.Mwfilter,self.save_ID,self.time_id,self.shuffle)
        #Get station information and ordering in X
        sta_loc_file=STAinfo['sta_loc_file']
        station_order_file=STAinfo['station_order_file']
        STAINFO=np.genfromtxt(sta_loc_file,'S12',skip_header=1)
        ...
        return X_batch[:,time_id,:],y_batch[:,time_id,:]
...
#    np.save('Run%s_test_EQID_all.npy'%(train_params['Testnum']),test_idx)
    ...
    #params for feature and label scaling
    Xscale=train_params['Xscale']
    yscale=train_params['yscale']
    ...
    if train_params['train_count']:
        ...
        network.add(layers.LeakyReLU(alpha=0.1))
        network.add(layers.Dropout(Drops[0]))
        #network.add(layers.LSTM(HP[2],return_sequences=True,input_shape=(None,NNsize,),dropout=0.1, recurrent_dropout=0.1))
        network.add(layers.LSTM(HP[2],return_sequences=True,input_shape=(None,NNsize,), recurrent_dropout=0.1))
        network.add(tf.keras.layers.TimeDistributed(layers.Dense(HP[3])))
        network.add(layers.LeakyReLU(alpha=0.1))
        network.add(tf.keras.layers.TimeDistributed(layers.Dense(HP[4])))
        network.add(layers.LeakyReLU(alpha=0.1))
    ...
    if not(os.path.exists('./Test'+Testnum)):
        os.makedirs('./Test'+Testnum)
    #Add callback
    CB=keras.callbacks.ModelCheckpoint(filepath='./Test'+Testnum+'/weights.{epoch:04d}-{val_loss:.6f}.hdf5',monitor='val_loss',save_best_only=True,mode='min',period=1)
    ...
    #start training
    #model_hist=network.fit_generator(gtrain,validation_data=(X_valid_out,y_valid_out),use_multiprocessing=True,workers=40,validation_steps=1,steps_per_epoch=1,epochs=epochs,callbacks=[CB,tensorboard_callback]) #so that total steps=1+7=8
    model_hist=network.fit_generator(gtrain,steps_per_epoch=min(len(X_train_E)//BS,300),validation_data=gvalid,validation_steps=min(len(X_valid_E)//BS_valid,10),use_multiprocessing=True,workers=64,epochs=epochs,callbacks=[CB,tensorboard_callback])
